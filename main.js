...
client.on("message", async message => {
    ...
    if (command === "サイコロ") {
        ...
        message.channel.send(
            Math.floor(Math.random() * 150) + 1 + "が出ました！"
        );
        return;
    }
        ...
        if (message.author.id == "500077489285103616") {
            ...
            if (!deleteCount || deleteCount < 2 || deleteCount > 100)
                return message.reply(
                    "削除するメッセージ数には2から100までの数字を入力してください"
                );
    ...
    if (command === "botinfo") {
        ...
        let embed = new Discord.RichEmbed()
            ...
            .addField(
                "BOTを招待する",
                "https://discordapp.com/api/oauth2/authorize?client_id=562820955723005962&permissions=271707376&scope=bot",
                false
            );
        message.channel.send(embed);
        return;
...
client.on("guildCreate", guild => {
    ...
    console.log(
        `新しいギルド参加： ${guild.name} (id: ${guild.id}). このギルドには ${
            guild.memberCount
        } 人が入っています！`
    );
});
...
client.on("guildMemberAdd", async member => {
    ...
    const background = await Canvas.loadImage(
        "https://cdn.glitch.com/cbda08d8-fddc-4b38-83d6-a334726c3b3c%2F2410.jpg"
    );
...
client.on("guildMemberRemove", async member => {
    ...
    // Since the image takes time to load, you should await it
    const background = await Canvas.loadImage(
        "https://cdn.glitch.com/cbda08d8-fddc-4b38-83d6-a334726c3b3c%2F3210.jpg"
    );
    // This uses the canvas dimensions to stretch the image onto the entire canvas
    ctx.drawImage(background, 0, 0, canvas.width, canvas.height);
